...
export const StatBadge = ({ label, value, accent, className = '' }) => (
  <div
    className={`group relative w-full overflow-hidden rounded-3xl border border-white/10 bg-white/5 p-6 backdrop-blur transition focus-within:ring-2 focus-within:ring-indigo-400/60 focus:outline-none hover:border-white/20 ${className}`.trim()}
  >
...
export const DataPanel = ({
  ...
  <section
    className={`relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/70 p-7 shadow-[0_30px_80px_-35px_rgba(59,130,246,0.35)] backdrop-blur-xl ${className}`.trim()}
  >
