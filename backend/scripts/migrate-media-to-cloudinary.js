...
const isMediaFile = (p) =>
  /(\.(jpg|jpeg|png|gif|webp|bmp|tiff|svg|mp4|webm|ogg|mov|mkv|mp3|wav|m4a|aac|flac|opus|wma|pdf|doc|docx|txt|xlsx|xls|ppt|pptx|zip|rar|7z))$/i.test(
    p
  );
...
async function ensureCloudinaryEnv() {
  ...
  if (!CLOUDINARY_CLOUD_NAME || !CLOUDINARY_API_KEY || !CLOUDINARY_API_SECRET) {
    console.error(
      "Cloudinary environment variables not set. Aborting migration."
    );
    process.exit(1);
  }
}
  ...
  try {
    const res = await cloudinary.uploader.upload(localFile, {
      ...
      public_id: `${Date.now()}_${path
        .basename(localFile, path.extname(localFile))
...
async function migrateMessageAttachments() {
  ...
  for await (const msg of cursor) {
    ...
    for (const att of msg.attachments || []) {
      ...
      if (isHttpUrl(att) && /res\.cloudinary\.com/.test(att)) {
        next.push(att);
        continue;
      }
    ...
    if (changed) {
      ...
      try {
        ...
        console.error("Save message failed", msg._id, e.message);
      }
    }
  }
  return updated;
  ...
  for (const post of posts) {
    ...
    if (changed) {
      try {
        ...
        console.error("Save post failed", post._id, e.message);
      }
    }
  }
  return updated;
  ...
  for (const ev of events) {
    ...
    if (url) {
      ...
      try {
        ...
        console.error("Save event failed", ev._id, e.message);
      }
    }
  }
  return updated;
  ...
  for (const u of users) {
    ...
    if (url) {
      ...
      try {
        ...
        console.error("Save user failed", u._id, e.message);
      }
    }
  }
  return updated;
  ...
  for (const doc of docs) {
    ...
    if (url) {
      ...
      try {
        ...
        console.error(`Save ${ModelName} failed`, doc._id, e.message);
      }
    }
  }
  return updated;
  ...
  for (const p of posts) {
    ...
    if (changed) {
      try {
        ...
        console.error("Save forum post failed", p._id, e.message);
      }
    }
  }
  return updated;
...
async function migrateFilesystemLooseFiles() {
  ...
  const folders = [
    UPLOADS_ROOT,
    path.join(UPLOADS_ROOT, "messages"),
    path.join(UPLOADS_ROOT, "profile"),
    path.join(UPLOADS_ROOT, "events"),
    path.join(UPLOADS_ROOT, "posts"),
    path.join(UPLOADS_ROOT, "forum"),
  ];
  let total = 0;
...
(async () => {
  ...
  results.students = await migrateProfiles(
    "Student",
    "profilePicture",
    "alumni-connect/avatars"
  );
  results.teachers = await migrateProfiles(
    "Teacher",
    "profilePicture",
    "alumni-connect/avatars"
  );
  results.alumni = await migrateProfiles(
    "Alumni",
    "profilePicture",
    "alumni-connect/avatars"
  );
